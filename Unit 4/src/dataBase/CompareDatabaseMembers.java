...
public class CompareDatabaseMembers {
 ...
 /*   // determines how many times it needs to loop through
    int minNumOfChars = firstStudentField.length();

    // converts to lowercase so upercase ascii value does not interfere with alphabetical sorting
    firstStudentField = firstStudentField.toLowerCase();
    secondStudentField = secondStudentField.toLowerCase();

    // if the second string is shorter, sets that as min iterations
    if (firstStudentField.length() > secondStudentField.length()) {
      minNumOfChars = secondStudentField.length();
    }
    ...
    for (int i = 0; i < minNumOfChars; i++) {
      if ((int) (secondStudentField.charAt(i)) < (int) firstStudentField.charAt(i)) {
        ordred = false;
        switched = true;
        break;
      } else if ((int) secondStudentField.charAt(i) > (int) firstStudentField.charAt(i)) {
        ordred = true;
        switched = true;
        break;
      }
    ...
    if (firstStudentField.length() > secondStudentField.length() && !switched) {
      ordred = false;
    }
  ...
  static public boolean compareStudentsByLastName(String firstName, String secondName, Student firstStudent, Student secondStudent) throws Exception {
